...
echo "compiling CRYT core..."
find src/java/nxt/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "CRYT core class files compiled successfully"

echo "compiling CRYT desktop..."
find src/java/nxtdesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "CRYT desktop class files compiled successfully"
else
    echo "if javafx is not supported, CRYT desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi
